...
class Memory(gvsoc.systree.Component):
    ...
    (preload, power-capture trigger, atomics) is identical, except
    that v3 models only a fixed per-request latency (no bandwidth /
    per-byte duration). Memcheck bookkeeping is *not* carried over:
    v3 is a plain backing store.
    ...
    Fully synchronous: every request is annotated with a single fixed
    latency (``cfg.latency`` cycles) via ``req->inc_latency()``, which
    the master reads inline under the IoV2Sync contract. There is no
    bandwidth / per-byte duration model — back-to-back accesses are
    each charged the same fixed latency. Model bandwidth with an
    ...
    A 64 KiB io_v2 memory with a 1-cycle fixed latency and atomic
    support:
    ...
    config : MemoryV3Config
        Full configuration. Every tunable — size, latency, stim_file,
    ...
    def i_INPUT(self) -> gvsoc.systree.SlaveItf:
        ...
        synchronously. The signature is :class:`IoV2Sync` — the memory
        always replies inline with ``IO_REQ_DONE`` and never goes async
        (no ``resp()`` / ``retry()``), though it does annotate
        ``req->latency``, which sync-aware masters read inline. This
        lets such masters elide their async/retry code paths.
        ``IoV2BigPacket`` and ``IoV2Beat`` masters bind here too
        (directly for big-packet, through the existing beat adapter for
        beat). v1 masters cannot bind to this port; use
