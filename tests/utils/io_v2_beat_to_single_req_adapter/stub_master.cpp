...
/*
 * Beat-master testbench for IoV2BeatToSingleReqAdapter. It declares signature
 * IoV2Beat on its output, so binding it to an IoV2SingleReq stub target makes
 * the framework auto-insert the adapter between them.
 *
...
private:
    ...
    // Request-path back-pressure: a burst whose req() the adapter DENIED, held
    // here and re-sent (synchronously) from retry_handler. Only writes are ever
    // denied upstream (the read path is absorbed by the adapter).
    BurstState *req_held = nullptr;
    // Response-path 1-beat/cycle arbiter: like router_v2_beat, this consumer
    // accepts at most one resp() beat per cycle and back-pressures the rest. The
    // cycle of the last accepted beat; a beat offered in the same cycle is
    // denied and re-driven next cycle via resp_retry().
...
void StubMaster::send_burst(BurstEntry *entry)
{
    ...
    // may check it). A read burst is data-less (beat protocol): the data comes
    ...
    // The adapter accepts (GRANTED, response streamed back via resp()) or
    // back-pressures the request (DENIED) — for a write whose forward was denied
    // downstream, or for a read once the adapter's read-burst limit is reached.
    // On DENIED we hold the burst and re-send it synchronously from
    // retry_handler. It never replies inline (DONE).
...
vp::IoRespAck StubMaster::resp_handler(vp::Block *__this, vp::IoReq *req)
{
    ...
    // A faithful beat consumer (cf. router_v2_beat) accepts at most one beat per
    // cycle. If a beat is offered in a cycle where we already accepted one — e.g.
    // the adapter catching up a backlog after earlier back-pressure — refuse it
    // and re-drive it on the next cycle via resp_retry().
    ...
    // Accept this beat (the arbiter above guarantees one per cycle).
    ...
    bool is_own = (req == bs->req);

    // ---- Ownership (initiator-owned request convention) ----
    // We own our request (bs->req) and free it ourselves on the last response;
    // nothing downstream frees it. Read responses are distinct allocator-backed
    // objects the adapter produces — free each back to its pool (req->free());
    // write acks round-trip our own request (req == bs->req), so that object is
    // freed once below as bs->req. The data buffer + per-burst state are ours.
    if (!is_own)
    {
    ...
    if (last)
    {
        ...
        delete bs->req;       // our request — we own it, free it on completion
        delete[] bs->buffer;
        delete bs;
        _this->outstanding--;
    }
...
void StubMaster::retry_handler(vp::Block *__this, vp::IoRetryChannel)
{
    ...
    // The adapter forwards retry() upstream whenever the downstream slave became
    // ready — even when we have nothing held (the read path is absorbed by the
    // adapter, which still forwards the retry). If we are holding a request the
    // adapter previously DENIED, re-send it now, synchronously, per the io_v2
    // contract.
    ...
    {
        // Still busy — keep holding and wait for the next retry().
    }
    else
    {
